        ...
        for i in range(len(xy)):
            ...
            for j in range(len(xy[0])):
                ...
                for k in range(len(xy[0])):
                    if j != k:
                        if edge[cnt, 0] == 1:
                            plt.plot([xy[i, j, 1], xy[i, k, 1]], [xy[i, j, 0], xy[i, k, 0]], color='black',
                                     linestyle='solid', lw=5)
                        cnt = cnt + 1
            plt.axis('off')
            writer.grab_frame();
            ax.cla()
                ...
                for k in range(len(xy[0])):
                    if j != k:
                        if edge[cnt, 1] == 1:
                            ...
                            plt.plot([xy[i, j, 1], xy[i, k, 1]], [xy[i, j, 0], xy[i, k, 0]], color='gray',
                                     linestyle='solid', lw=5)
                        elif edge[cnt, 3] == 1:
                            plt.plot([xy[i, j, 1], xy[i, k, 1]], [xy[i, j, 0], xy[i, k, 0]], color='purple',
                                     linestyle='solid', lw=5)
...
def Test_TPN(dataset, Fn, frame_len, relation_threshold):
    ...
    for timestep in range(frame_len):
        ...
        for m in range(n_objects):
            for j in range(n_objects):
                if (m != j):
                    inzz = np.linalg.norm(GroundData2[:, timestep, m, 2:4] - GroundData2[:, timestep, j, 2:4],
                                          axis=1) < relation_threshold
            ...
            for j in range(len(xy[0])):
                ...
                for k in range(len(xy[0])):
                    if j != k:
                        if edge[cnt, i, 0] > 0.5:
                            plt.plot([xy[i, j, 1], xy[i, k, 1]], [xy[i, j, 0], xy[i, k, 0]], color='black',
                                     linestyle='solid', alpha=edge[cnt, i, 0], lw=5)
                        cnt = cnt + 1
            plt.axis('off')
            writer.grab_frame();
            ax.cla()
            ...
            for j in range(len(xy[0])):
                ...
                for k in range(len(xy[0])):
                    if j != k:
                        ...
                        if edge_type == 1:
                            plt.plot([xy[i, j, 1], xy[i, k, 1]], [xy[i, j, 0], xy[i, k, 0]], color='black',
                                     linestyle='solid', alpha=edge[cnt, i, 1], lw=5)
                        elif edge_type == 2:
                            plt.plot([xy[i, j, 1], xy[i, k, 1]], [xy[i, j, 0], xy[i, k, 0]], color='gray',
                                     linestyle='solid', alpha=edge[cnt, i, 2], lw=5)
                        elif edge_type == 3:
                            plt.plot([xy[i, j, 1], xy[i, k, 1]], [xy[i, j, 0], xy[i, k, 0]], color='purple',
                                     linestyle='solid', alpha=edge[cnt, i, 3], lw=5)
                        cnt = cnt + 1
            plt.axis('off')
            writer.grab_frame();
            ax.cla()

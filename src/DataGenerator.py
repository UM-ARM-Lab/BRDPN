...
class RelationDataGeneratorMany(tf.keras.utils.Sequence):
    'Generates data for Keras'

    def __init__(self, n_objects, n_of_rel_type, number_of_frames, num_of_traj, dataset, dataRelation,
                 relation_threshold, isTrain=True, batch_size=100, timestep_diff=5, lookout=50, shuffle=True):
        'Initialization'
        self.n_objects = n_objects
        self.relation_threshold = relation_threshold
        self.n_of_rel_type = n_of_rel_type
        ...
        for i in range(1, self.num_of_traj):
            self.indexes = np.concatenate([self.indexes, (i * self.number_of_frames + timestep_diff + 1 + np.arange(
                self.number_of_frames - timestep_diff - 1))])
        self.on_epoch_end()
    ...
    def extract_traj_data(self, n_of_frame, traj_index):
        data_indexes = self.indexes[traj_index * (self.number_of_frames - self.timestep_diff - 1):traj_index * (
                    self.number_of_frames - self.timestep_diff - 1) + n_of_frame]
    ...
    def __data_generation(self, data_indexes, batch_size=0):
        ...
        self.x_receiver_relations = np.zeros((batch_size, self.timestep_diff, self.n_objects, self.n_relations),
                                             dtype=float);
        self.x_sender_relations = np.zeros((batch_size, self.timestep_diff, self.n_objects, self.n_relations),
                                           dtype=float);
        ...
        for timestep in range(self.timestep_diff):
            ...
            for i in range(self.n_objects):
                for j in range(self.n_objects):
                    if (i != j):
                        inzz = np.linalg.norm(temp_data[:, timestep, i, 2:4] - temp_data[:, timestep, j, 2:4],
                                              axis=1) < self.relation_threshold
        ...
        return {'objects':            x_object, 'diff_objects': x_diff_object,
                'sender_relations':   self.x_sender_relations, \
                'receiver_relations': self.x_receiver_relations, \
                'propagation':        self.propagation, 'known_relations': knownRelations,
                'unknown_relations':  unknownRelations, 'relation_dummy': relation_dummy}, {
                   'target': self.relation_info2}

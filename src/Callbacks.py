                ...
                for j in range(num_of_object):
                    if (m != j):
                        inzz = np.linalg.norm(dataToModel[:, 0, m, 2:4] - dataToModel[:, 0, j, 2:4],
                                              axis=1) < self.scaler.relation_threshold
                        ...
                        if self.n_of_rel > 1:
                            ...
                            val_relation_info[
                                np.sum(val_relation_info[:, cnt, 1:self.n_of_rel], axis=1) == 0, cnt, 0] = 1
                        else:
                            val_relation_info[:, cnt, :] = self.val_relation_info[k][:, 0, m * num_of_object + j, :]
                        cnt += 1
            for i in range(1, 100):
                velocities = Pn.predict(
                    {'objects':            dataToModel[:, i - 1, :, :], 'sender_relations': val_sender_relations,
...
class PlotLosses(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        for z in range(self.n_of_dataset):
            ...
            print(str(self.num_of_objects[z]) + ' objects vel loss:',
                  logs.get('val' + str(self.num_of_objects[z]) + '_vel_loss'))

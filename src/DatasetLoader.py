...
class MyScaler:
    ...
    def __repr__(self):
        return '\n'.join([str(s) for s in [self.relation_threshold, self.multiplier, self.inv_multiplier]])
...
class MyDataset:
            ...
            if os.path.isfile(self.PATH + str(scene_ind) + '/info2.txt'):
                ...
                for obj_ind in range(self.n_of_obj - 3):
                    if temp_txt2[obj_ind, 0] > 1:
                        ...
                        self.r_i[scene_ind * self.n_of_exp:(scene_ind + 1) * self.n_of_exp, :,
                        obj1 * (self.n_of_obj + 1) + obj2, int(temp_txt2[obj_ind, 0] - 2)] = 1
                        self.r_i[scene_ind * self.n_of_exp:(scene_ind + 1) * self.n_of_exp, :,
                        obj2 * (self.n_of_obj + 1) + obj1, int(temp_txt2[obj_ind, 0] - 2)] = 1
    ...
    def divideDataset(self, tr_size, val_size):
        ...
        self.r_i_val[:, :] = self.r_i[tr_set_size:(tr_set_size + val_set_size)].reshape(-1, (self.n_of_obj + 1) ** 2,
                                                                                        self.num_of_rel_type)
        self.r_i_test[:, :] = self.r_i[self.n_of_traj - test_set_size:].reshape(-1, (self.n_of_obj + 1) ** 2,
                                                                                self.num_of_rel_type)

        self.data_tr[:, :, :] = self.data[:tr_set_size].reshape(-1, (self.n_of_obj + 1), self.f_size)
        self.data_val[:, :, :] = self.data[tr_set_size:(tr_set_size + val_set_size)].reshape(-1, (self.n_of_obj + 1),
                                                                                             self.f_size)
            ...
            if os.path.isfile(self.PATH + str(scene_ind) + '/info2.txt'):
                ...
                for rel_ind in range(self.n_of_groups):
                    if temp_txt2[rel_ind, 0] > 1:
                        ...
                        self.r_i[scene_ind * self.n_of_exp:(scene_ind + 1) * self.n_of_exp, :,
                        obj1 * (self.n_of_obj + 1) + obj2, int(temp_txt2[rel_ind, 0] - 2)] = 1
                        self.r_i[scene_ind * self.n_of_exp:(scene_ind + 1) * self.n_of_exp, :,
                        obj2 * (self.n_of_obj + 1) + obj1, int(temp_txt2[rel_ind, 0] - 2)] = 1

from keras.layers import Permute, Subtract, Add, Lambda, Input, Concatenate, TimeDistributed, Activation, Dropout, dot, \
    Reshape
...
class PropagationNetwork:
    ...
    def getModel(self, n_objects, object_dim=6, relation_dim=1):
        ...
        predicted = no_hand(x)
        model = Model(inputs=[objects, sender_relations, receiver_relations, relation_info, propagation],
                      outputs=[predicted])
...
class TemporalPropagationNetwork:
    ...
    def getModel(self, n_objects, object_dim=6, relation_dim=1, timestep_diff=100, lookstart=50):
        ...
        if (self.set_weights):
            ...
            rme = RecurrentRelationalModel_many(n_relations, 100, [100, relation_dim], self.rmepart1, self.rmepart2,
                                                True, timestep_diff, lookstart)
        else:
            ...
            rme = RecurrentRelationalModel_many(n_relations, 100, [100, relation_dim], None, None, False, timestep_diff,
                                                lookstart)
            self.set_weights = True
            self.relnet = rm.getRelnet()
            self.objnet = om.getObjnet()
        ...
        predicted = Lambda(lambda x: x[:, :, :],
                           output_shape=(n_relations, timestep_diff - start_step, relation_dim), name='target')(
            predicted)
        model = Model(
            inputs=[objects, relation_data, sender_relations_in, receiver_relations_in, propagation, known_relations,
                    unknown_relations, relation_dummy], outputs=[predicted])
        adam = optimizers.Adam(lr=0.0001, decay=0.0)
        if relation_dim == 1:
